...
The application listens on `9092` for HTTP and
`9202` for gRPC by default. Set
`INVENTORY_SERVICE_HTTP_PORT` or
`INVENTORY_SERVICE_GRPC_PORT` to change the listener and
container-side mapping. Set `INVENTORY_SERVICE_HOST_HTTP_PORT`
or `INVENTORY_SERVICE_HOST_GRPC_PORT` to change only the
forwarded host port.
